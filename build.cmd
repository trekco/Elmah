...
:net-4-0
call :compile v4.0.30319 net-4.0 /d:NET_4_0 /r:lib\System.Data.SQLite.dll /r:lib\Npgsql.dll /r:lib\mysql.data.dll /r:lib\System.Data.SqlServerCe.dll /nowarn:618
call :deps 4.0
goto :EOF
...
set CSC_COMMON=/unsafe- /checked- /warnaserror+ /nowarn:1591,618 /warn:4 /d:TRACE /debug+ /baseaddress:285212672 /r:Microsoft.JScript.dll 
...
echo.
echo TARGET
echo     is the target to build (all, 4.0)
echo.
echo This is a batch script that can used to build ELMAH binaries for 
echo Microsoft .NET Framework 4.0. The binaries are created for 
...
echo.
for %%i in (v4.0.30319) do if exist "%NETFX_BASE_PATH%\%%i\csc.exe" echo - %%i
call :lic
goto :EOF
