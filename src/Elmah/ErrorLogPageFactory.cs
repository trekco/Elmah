...
namespace Elmah
{
    ...
    public class ErrorLogPageFactory : IHttpHandlerFactory
    {
        ...
        public virtual IHttpHandler GetHandler(HttpContext context, string requestType, string url, string pathTranslated)
        {
            ...
            if (authorized == 0 || (authorized < 0 && !HttpRequestSecurity.IsLocal(context.Request) && !SecurityConfiguration.Default.AllowRemoteAccess))
            {
            ...
            if (resource != "stylesheet" && resource != "login" && SecurityConfiguration.Default.RequireLogin && !HttpRequestSecurity.LoggedOnUsername(context.Request).Equals(SecurityConfiguration.Default.UserName, StringComparison.CurrentCultureIgnoreCase))
            {
                context.Response.Redirect(context.Request.AppRelativeCurrentExecutionFilePath + "/Login",true);
            }
